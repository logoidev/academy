...
const EMPTY_FIELD = new Array(FIELD_SIZE)
  .fill(0)
  .map((_) => new Array(FIELD_SIZE).fill(0));
...
const run = async () => {
  ...
  const checkRow = (rowIndex, checkFor = 1) =>
    field[rowIndex].every((item) => item === checkFor);

  const checkColumn = (columnIndex, checkFor = 1) =>
    field.every((row, j) => row[columnIndex] === checkFor);

  const checkFirstDiagonal = (checkFor = 1) =>
    field.every((_, i) => field[i][i] === checkFor);
  ...
  const drawField = () => {
    console.clear();
    const result = field
      .map((item) => item.map(indexToCharacter).join("|"))
      .join("\n");
    console.log(result);
  };
  ...
  const askForIndexes = (index) => {
    ...
    const isValid = ([x, y, ...rest]) => {
      ...
      if (field[x][y] !== 0) {
        drawField();
        throw new Error(
          `Cell ${x} ${y} not empty -> ${indexToCharacter(field[x][y])}`
        );
      }
      return true;
    };
