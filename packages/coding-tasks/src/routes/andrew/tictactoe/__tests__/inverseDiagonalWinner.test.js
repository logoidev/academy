...
describe('getInverseDiagonal', () => {
	...
	describe('getDiagonalWinner', () => {
		...
		it('minimal lengths', () => {
			...
			expect(printField(array, getInverseDiagonal(array, 1, 0).indexes)).toMatchInlineSnapshot(`
          "o|x
          x|o"
        `);

			expect(printField(array, getInverseDiagonal(array, 1, 1).indexes)).toMatchInlineSnapshot(`
          "o|x
          x|o̶"
        `);
		});
